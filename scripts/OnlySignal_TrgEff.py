...
# 
        ...
        'genLeadingMuPt':{'hname':"h_genLeadingMuPt",'label':"leading #mu Gen. p_{T} [GeV]",'xlow':0,'xhigh':100,'hrebin':50},
        'gensubLeadingMuPt':{'hname':"h_gensubLeadingMuPt",'label':"leading #mu Gen. p_{T} [GeV]",'xlow':0,'xhigh':100,'hrebin':50},
    ...
    datasets_dict = {
        ...
        # 'HToSS_MH125_MS'+args.mass+'_ctauS100':{'type':'signal','label':"#splitline{m_{S}="+args.mass.replace('p','.')+" GeV,}{c#tau = 100mm}",'style': 33,'color':6,'integral':-1},
        ...
        # 'HToSS_MH125_MS'+args.mass+'_ctauS1_ctauS0p1':{'type':'signal','label':"#splitline{m_{S}="+args.mass.replace('p','.')+" GeV,}{c#tau = 1mm#rightarrow 0.1mm}",'color':ROOT.kGray+3,'integral':-1},
        ...
        # 'HToSS_MH125_MS'+args.mass+'_ctauS10_ctauS1':{'type':'signal','label':"#splitline{m_{S}="+args.mass.replace('p','.')+" GeV,}{c#tau =10mm#rightarrow 1mm}",'color':ROOT.kOrange+1,'integral':-1},
        # 'HToSS_MH125_MS'+args.mass+'_ctauS100_ctauS10':{'type':'signal','label':"#splitline{m_{S}="+args.mass.replace('p','.')+" GeV,}{c#tau =100mm#rightarrow 10mm}",'color':ROOT.kRed+1,'integral':-1},
    ...
    trg_eff = ["singlemutrg","l2mutrg","mutrg"]
    trg_eff_ylabel = ["#mu trigger efficiency","L2/no vtx #mu#mu trigger efficiency","L2/no vtx #mu#mu OR #mu trigger efficiency"]
    ...
    for num,trg in enumerate(trg_eff):
        for key in histo_dict:
            ...
            # hsig=[]
            if args.analysis:
                savename=key+'_'+'MS'+args.mass+"_analysis"
            else:
                savename=key+'_'+'MS'+args.mass+"_"+trg
