//An example macro for manipulating 2D plots
 ...
 #include "TLatex.h"
  ...
  void examplePlot2DMacro() {  
     ...
     TH2I* h_VertexPosXY = (TH2I*)inFile.Get("VertexPosXY"); // Load in histogram
     ...
     h_VertexPosXY->GetXaxis()->SetRangeUser(0.0, 50.); // change the x-axis range to 0-200
     h_VertexPosXY->GetYaxis()->SetTitle("Vertex Position y (mm)"); 
     h_VertexPosXY->GetYaxis()->SetRangeUser(0.0, 50.); //
