...
def TotalEvents(key,fname,plot_src,category,hname,lo_bound,hi_bound,eff_dict,apply_eff=True):
    # fname = datasets_dict[key]['fname']
    ...
    # Signal modelling
    f_mc = ROOT.TFile(plot_src+"/"+fname,"r")
    hname="eventFilterAND"
    h_skim=f_mc.Get(hname).Clone()
    h_skim.Scale(lumi)
    totevts=h_skim.GetBinContent(1)
    return totevts
...
def main():
    ...
    for category in categories:
        ...
        # yields_tot[category]={}
        # yields_vbf_tot[category]={}
        for ct in ctaus:
            fname='output_HToSS_MH125_MS'+m_point+'_ctauS'+ct+'.root'
            fname_vbf='output_VBF_HToSS_MH125_MS'+m_point+'_ctauS'+ct+'.root'
            ...
            if 'prompt' in category:
                sig_total=TotalEvents(key,fname,plot_src,category,hname,lo_bound,hi_bound,feff_fit_dict)
            sig_rate_vbf,sig_err_vbf=SignalYield(key,fname_vbf,plot_src,category,hname,lo_bound,hi_bound,feff_fit_dict,False)
            if 'prompt' in category:
                sig_total_vbf=TotalEvents(key,fname_vbf,plot_src,category,hname,lo_bound,hi_bound,feff_fit_dict)
            print('SEETHIS:',sig_rate,sig_err)
            yields[category][ct]=sig_rate
            yields_vbf[category][ct]=sig_rate_vbf
            yields_err[category][ct]=sig_err
            yields_vbf_err[category][ct]=sig_err_vbf
            if 'prompt' in category:
                yields_tot[ct]=sig_total
                yields_vbf_tot[ct]=sig_total_vbf
    ...
    for ct in ctaus:
        ...
        eff_inc[ct]=round(yield_vbf_sum[ct]/yield_sum[ct]*100,2)
        try:
            signal_eff[ct]=yield_sum[ct]/yields_tot[ct]*100
            signal_eff_vbf[ct]=yield_vbf_sum[ct]/yields_vbf_tot[ct]*100
    ...
    print(eff_inc)
    print(yields_tot)
    print(yields_vbf_tot)
    print("Signal efficiency")
    print(signal_eff)
    print(signal_eff_vbf)
    
    fyaml=open('VBF_effect.yaml','r')
    yaml_d = yaml.safe_load(fyaml)
    fyaml.close()
    yaml_d['MS'+args.mass]['ggH']={}
    yaml_d['MS'+args.mass]['VBF']={}
    yaml_d['MS'+args.mass]['inc']={}
    for ct in ctaus:
        yaml_d['MS'+args.mass]['ggH'][ct]=[float(yield_sum[ct]),float(yield_sum_err[ct]),round(signal_eff[ct],1)]
        yaml_d['MS'+args.mass]['VBF'][ct]=[float(yield_vbf_sum[ct]),float(yield_vbf_sum_err[ct]),round(signal_eff_vbf[ct],1)]
        yaml_d['MS'+args.mass]['inc'][ct]=[float(eff_inc[ct]),float(eff_inc_err[ct])]
    print(yaml_d)
    fyaml=open('VBF_effect.yaml','w')
    yaml.safe_dump(yaml_d, fyaml, sort_keys=False, default_flow_style=False)
    fyaml.close()
    fout=open('VBF_compare.txt','w')
    latex_code = format_latex_table(yields)
    fout.write('VBF; MS'+args.mass)
    fout.write('\n')
