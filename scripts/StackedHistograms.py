...
def main():
    ...
    histo_dict = {
        ...
        'recoHiggsMass_All_WithPeak': {'hname':'h_recoHiggsMass_All_WithPeak','label':"m_{h^{+}h^{-}#mu^{+}#mu^{-}} (GeV)",'xlow':110,'xhigh':140,'hrebin':5},
        ...
        'recoHiggsMass_HMassBC': {'hname':'h_recoHiggsMass_HMassBC','label':"m_{h^{+}h^{-}#mu^{+}#mu^{-}} (GeV)",'xlow':110,'xhigh':140,'hrebin':5},
        # 'recoHiggsMass_SMassBC': {'hname':'h_recoHiggsMass_SMassBC','label':"m_{h^{+}h^{-}#mu^{+}#mu^{-}} (GeV)",'xlow':115,'xhigh':135,'hrebin':5},
        'AvgMass_mumu_hh':{'hname':'h_AvgMass_mumu_hh','label':"0.5*(m_{hh}+m_{#mu#mu}) (GeV)",'xlow':0.5,'xhigh':3.5,'hrebin':40},
        'AvgMass_mumu_hh_HMassBC':{'hname':'h_AvgMass_mumu_hh_HMassBC','label':"0.5*(m_{hh}+m_{#mu#mu}) (GeV)",'xlow':0.5,'xhigh':3.5,'hrebin':40},
        'AvgMass_mumu_hh_HMassBC_FineBinning':{'hname':'h_AvgMass_mumu_hh_HMassBC','label':"0.5*(m_{hh}+m_{#mu#mu}) (GeV)",'xlow':1.8,'xhigh':2.2,'hrebin':1},
        'AvgMass_mumu_hh_HMassBC_TF':{'hname':'h_AvgMass_mumu_hh_HMassBC','label':"0.5*(m_{hh}+m_{#mu#mu}) (GeV)",'xlow':0.5,'xhigh':3.5,'hrebin':40},
    ...
    #datasets_label = ["#splitline{m_{a}="+sig_mass+" GeV,}{ c#tau = "+sig_ctau+"mm}","VVV", "t#bar{t}V", "Single top", "VV", "t#bar{t}", "Z+jets", "W+jets","QCD"]
    ...
    for dirn in dirname:
        for key in histo_dict:
            # if ("h_recoHiggsMass" != histname):
                # continue
            folder = dirn
            hprop = histo_dict[key]
            histname = hprop['hname']
            # if ('Avg' not in histname):
            #     continue
            # if (histname!='Cutflow'):
            #     continue
            # if 'Hits' not in histname:
            #     continue
            # if ('recoHiggsMass_WithPeak' not in histname) and ('Significance' not in histname):
            #     continue
            # if 'recoHiggsMass_WithPeak_ScalarCut' not in histname:
            #     continue
            ...
            if "FineBinning" in key:
                savename = savename + "_Finebinning"
            ...
            if args.withbkg:
                for i, dataset in enumerate(datasets_bkg):
                    ...
                    print(histname)
                    #print(hs[0].GetName())
                    h_0 = fh.Get(dirn+'/'+histname).Clone()
                    # nBins = h_0.GetNbinsX()
                    ...
                    if ("Higgs" not in histname):
                        ...
                        # print("Current bins:",tst[tst>=120])
                        h_0 = h_0.Rebin(len(custom_bins)-1,"h_recoHiggsMass_rebinned",custom_bins)
                        # for k in range(1,h_0.GetNbinsX()+1):
                        #     binXval = h_0.GetXaxis().GetBinLowEdge(k)
                        #     if (binXval < 122.5) or (binXval > 127.5):
                        #         continue
                        #     h_0.SetBinContent(k,0.)
                    ...
                    hs.Add(h_0)
                    # for nx in range(h_0.GetNbinsX()):
                    #     h_0.SetBinContent(nx,0)
            ...
            fsig = ROOT.TFile(args.input+"/"+"output_" + args.sig + ".root", "READ")
            #fsig = ROOT.TFile(args.input+"/"+datasets[sig_ind[0].item()] + ".root", "READ")
            h_1 = fsig.Get(dirn+'/'+histname).Clone()
            if args.mult != 1:
                h_1.Scale(args.mult)
            if ("Higgs" not in histname):
            ...
            if args.withdata:
                ...
                # if ("_BC" in histname):
                #     histname = histname.replace("_BC","")
                #     if ("_RelIso" in histname):
                #         histname = histname.replace("_RelIso","RelIso")
                print("LOOK HERE:",histname)
                if "recoHiggsMass" in histname:
                    histname_tmp = histname.replace('_WithPeak','')
                    h_2 = fdata.Get(dirn+'/'+histname_tmp).Clone()
                # if ('WithPeak_ScalarCut' in histname):
                #     h_2 = fdata.Get('h_recoHiggsMass_HMassBC').Clone()
                # elif ('recoHiggsMass_WithPeak' in histname):
                #     h_2 = fdata.Get('h_recoHiggsMass_HMassSMassBC').Clone()
                else:
                    h_2 = fdata.Get(dirn+'/'+histname).Clone()
                if ("Higgs" not in histname):
