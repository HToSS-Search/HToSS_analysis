...
parser.add_argument("--vbf",dest="vbf", help="VBF or not; false by default", action="store_true")
...
for conf in pars['datasets']:
	conf_og=conf
	mS=conf.split('/')[-1].split('MS')[-1].split('_')[0]
	lt=conf.split('/')[-1].split('_')[-1].replace('.yaml','').replace('ctauS','')
	if conf.count('ctauS') == 2:
		og_lt = conf.split('/')[-1].split('_')[-2].replace('ctauS','')
		lt=og_lt
		# print(new_lt)
		if 'pion' in conf:
	...
	if conf.count('ctauS') == 3:
		...
		# quit()
	if args.vbf:
		f_conf=open(conf.split('VBF/')[0]+'vbf.yaml','r')
		conf_pars = yaml.safe_load(f_conf)
		data_loc = conf_pars['MS'+mS]['dir']
		list_of_files=[]
		...
		for fin in all_files:
			if ('_mS'+mS.replace('p','.')+'_' not in fin) or ('_ctauS'+lt+'_' not in fin):
				# print("enters")
				continue
			full_path = os.path.join(data_loc, fin)
			# print(full_path)
			if os.path.isfile(full_path):
				list_of_files.append(full_path)
		# quit()
		print(list_of_files)
		nmax=len(list_of_files)
		print(nmax)
		out_name = conf_og.split("/")[-1].split(".yaml")[0].replace('_pion','')
		data_name = 'VBF_'+conf_og.split('/')[-1].replace('.yaml','').replace('_pion','')
	...
	if (args.justWeights): # generating only weight distribution
		...
		if 'HToSS' not in data_name:
			...
			if args.vbf:
				os.makedirs(cwd+'/'+args.outdir+'/'+'VBF', exist_ok=True)
			else:
	...
	for i in range(1, nmax+1, njobs):
		...
		if (args.justWeights): # generating only weight distribution
			...
			if 'HToSS' in out_name:
				if args.vbf:
					line = conf.split('VBF/')[0]+'vbf.yaml' + "," + args.cuts + "," +args.outdir+"/"+"VBF"+"/output_VBF_"+out_name + ".root" + "," + start + "," + end + "," + data_name + ','+ args.year
				else:
